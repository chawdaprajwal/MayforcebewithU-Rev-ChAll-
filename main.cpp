...
bool ultimate_verification(const std::string& input, uint64_t intermediate_hash) {
    ...
    uint64_t final_hash = layer1 ^ layer2 ^ layer3 ^ layer4;
...
if (ida || ghidra || x64dbg || olly || windbg) {
    ...
    if (result) {
        ...
        std::cout << "  ???     ????????  ?????  ??????? ??????? ??????????????? \n";
        std::cout << "\n";
        std::cout << "  [?] CONGRATULATIONS! You've cracked it!\n";
        std::cout << "\n";
        std::cout << "  FLAG: BTCTF{" << input << "}\n";
        std::cout << "\n";
        std::cout << "  You are a true reverse engineering master!\n";
        std::cout << "  The dialogue was yours to discover.\n";
        std::cout << "\n";
    } else {
        std::cout << "\n";
        std::cout << "  ? ACCESS DENIED\n";
        std::cout << "  The secret eludes you...\n";
        std::cout << "\n";
    }
